...
Copied with modifications from https://github.com/Nervengift/read_thermal.py/blob/master/flir_image_extractor.py
...
from util.get_weather_data import get_temp
import pathlib
...
class FlirImageExtractor:
    ...
    def __init__(self, exiftool_path="exiftool", is_debug=False):
        ...
        self.image_suffix = "_visual.jpg"
        self.thumbnail_suffix = "_thumb.jpg"
    ...
    def process_image(self, flir_img_filename):
        ...
        self.thermal_image_np = self.extract_thermal_image_historical()
    ...
    def extract_thermal_image_historical(self):
        """
        extracts the thermal image as 2D numpy array with temperatures in oC using historical temp values
        """
        ...
        # Fresno state: 36.8137854531408, -119.74619741145872
        hist_temp, hist_rh = get_temp(pathlib.Path(self.flir_img_filename), 36.8137854531408, -119.74619741145872)
        if self.is_debug:
            print("DEBUG Flir image historical temperature:{}, historical relative humidity:{}".format(hist_temp, hist_rh))
